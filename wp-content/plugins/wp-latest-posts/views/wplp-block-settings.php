...
class WPLP_Block_Settings
{
    ...
    private $_field_defaults = false;

    public function __construct($default)
    {
        $this->_field_defaults = $default;
        ...
        <div style="background:#fff; border: none;" class="ui-tabs ui-widget ui-widget-content ui-corner-all">
            ...
            <div id="wplpnavtabs" class="wplptabs" style="display: none">
                <ul class="tabs z-depth-1">
                    <li class="tab"><a href="#tab-1" id="content-source" data-tab-id="content-source"><?php _e('Content source', 'wp-latest-posts'); ?></a></li>
                    <li class="tab"><a href="#tab-2" id="display-theme" data-tab-id="display-theme"><?php _e('Display and theme', 'wp-latest-posts'); ?></a></li>
                    <li class="tab"><a href="#tab-3" id="image-source" data-tab-id="image-source"><?php _e('Images source', 'wp-latest-posts'); ?></a></li>
                    <li class="tab"><a href="#tab-4" id="advanced" data-tab-id="advanced"><?php _e('Advanced', 'wp-latest-posts'); ?></a></li>
                </ul>
    ...
    private function displayImageSourceTab()
    {
        ...
        if (strpos($settings["theme"], 'masonry-category') !== false || strpos($settings["theme"], 'masonry') !== false || strpos($settings["theme"], 'smooth-effect') !== false || strpos($settings["theme"], 'portfolio') !== false || strpos($settings["theme"], 'timeline') !== false) {
        ...
        */
        echo '<li class="field ' . $class_enabled_default . '"><label for="image_position_width" class="coltab wplp_tooltip" alt="'.__('The image width in %, used when the image is loaded on left position','wp-latest-posts').'">' . __('Image position width', 'wp-latest-posts') . '</label>' .
            '<input id="image_position_width" ' . $disabled_image_postion . ' size="20" type="text" style="width:50px" name="wplp_image_position_width" value="' . htmlspecialchars(isset($settings['image_position_width']) ? $settings['image_position_width'] : '') . '" class="short-text ' . $class_enabled_default . '" />';
        ...
        echo '<li class="field"><label for="margin_left" class="coltab">' . __('Margin left', 'wp-latest-posts') . '</label>' .
            ...
            '<input id="margin_left" type="text" name="wplp_margin_left" value="' . htmlspecialchars(isset($settings['margin_left']) ? $settings['margin_left'] : '') . '" class="short-text" /></li>';
        echo '<li class="field"><label for="margin_top" class="coltab">' . __('Margin top', 'wp-latest-posts') . '</label>' .
            ...
            '<input id="margin_top" type="text" name="wplp_margin_top" value="' . htmlspecialchars(isset($settings['margin_top']) ? $settings['margin_top'] : '') . '" class="short-text" /></li>';
        echo '<li class="field"><label for="margin_right" class="coltab">' . __('Margin right', 'wp-latest-posts') . '</label>' .
            ...
            '<input id="margin_right" type="text" name="wplp_margin_right" value="' . htmlspecialchars(isset($settings['margin_right']) ? $settings['margin_right'] : '') . '" class="short-text" /></li>';
        echo '<li class="field"><label for="margin_bottom" class="coltab">' . __('Margin bottom', 'wp-latest-posts') . '</label>' .
            ...
            '<input id="margin_bottom" type="text" name="wplp_margin_bottom" value="' . htmlspecialchars(isset($settings['margin_bottom']) ? $settings['margin_bottom'] : '') . '" class="short-text" /></li>';
        echo '</ul>'; //fields
    ...
    private function displayAdvancedTab()
    {
        ...
        echo '<li class="field"><label for="date_fmt" class="coltab">' . __('Date format', 'wp-latest-posts') . '</label>' .
            '<input id="date_fmt" type="text" name="wplp_date_fmt" value="' . htmlspecialchars(isset($settings['date_fmt']) ? $settings['date_fmt'] : '') . '" class="short-text" />
			<a id="wplp_dateFormat" target="_blank" href="http://php.net/manual/en/function.date.php"> ' . __('Date format', 'wp-latest-posts') . ' </a>
			</li>';
        ...
        echo '<li class="field"><label for="no_post_text" class="coltab">' . __('No post text', 'wp-latest-posts') . '</label>' .
            '<input id="no_post_text" type="text" name="wplp_no_post_text" value="' . htmlspecialchars(isset($settings['no_post_text']) ? $settings['no_post_text'] : '') . '" class="short-text" /></li>';


        echo '</ul>'; //fields

        echo '<hr/><div><label for="custom_css" class="coltab" style="vertical-align:top">' . __('Custom CSS', 'wp-latest-posts') . '</label>' .
            '<textarea id="wplp_custom_css" cols="100" rows="5" name="wplp_custom_css">' . (isset($settings['custom_css']) ? $settings['custom_css'] : '') . '</textarea></div>';
        ...
        if (isset($post->ID) && isset($post->post_title) && (!empty($post->post_title))) {
            echo '<hr style="clear:both"/><div><label for="phpCodeInsert" class="coltab" style="margin:10px 0 5px">' . __('Copy & paste this code into a template file to display this WPLP block', 'wp-latest-posts') . '</label>' .
                '<br><textarea readonly id="phpCodeInsert" cols="100" rows="2" name="wplp_phpCodeInsert">' . 'echo do_shortcode(\'[frontpage_news widget="' . $post->ID . '" name="' . $post->post_title . '"]\');' . '</textarea></div>';
        }
    }
    ...
    private function displayDisplayThemeTab()
    {
        ...
        echo '<li class="field"><label class="coltab">' . __('Show title', 'wp-latest-posts') . '</label>' .
            '<span class="radioset">' .
            '<input id="show_title1" type="radio" name="wplp_show_title" value="0" ' . (isset($show_title_checked[0]) ? $show_title_checked[0] : '') . '/>' .
            '<label for="show_title1">' . __('Off', 'wp-latest-posts') . '</label>' .
            '<input id="show_title2" type="radio" name="wplp_show_title" value="1" ' . (isset($show_title_checked[1]) ? $show_title_checked[1] : '') . '/>' .
        ...
        echo '<li class="field ' . $classdisabledsmooth . '"><label for="   amount_cols" class="coltab">' . __('Number of columns', 'wp-latest-posts') . '</label>' .
            '<input id="amount_cols" type="text" name="wplp_amount_cols" value="' . htmlspecialchars(isset($settings['amount_cols']) ? $settings['amount_cols'] : '3') . '" class="short-text" ' . $classdisabledsmooth . '/></li>';
        /*
        ...
        echo '<li class="field ' . $classdisabled . $classdisabledsmooth . '"><label for="amount_rows" class="coltab">' . __('Number of rows', 'wp-latest-posts') . '</label>' .
            '<input id="amount_rows" type="text" name="wplp_amount_rows" value="' . htmlspecialchars(isset($settings['amount_rows']) ? $settings['amount_rows'] : '') . '" class="short-text" ' . $classdisabled . $classdisabledsmooth . '/></li>';
        ...
        echo '<li class="field ' . $classdisabled . $classdisabledsmooth . '"><label for="pagination" class="coltab">' . __('Pagination', 'wp-latest-posts') . '</label>' .
            '<select id="pagination" name="wplp_pagination" class="browser-default ' . $classdisabled . $classdisabledsmooth . '" >';
        ...
        echo '<li class="field"><label for="max_elts" class="coltab">' . __('Max number of elements', 'wp-latest-posts') . '</label>' .
            '<input id="max_elts" type="text" name="wplp_max_elts" value="' . htmlspecialchars(isset($settings['max_elts']) ? $settings['max_elts'] : '') . '" class="short-text" /></li>';
        /*
         * display total width
         */
        echo '<li class="field"><label for="total_width" class="coltab">' . __('Total width', 'wp-latest-posts') . '</label>' .
            '<input id="total_width" type="text" name="wplp_total_width" value="' . htmlspecialchars(isset($settings['total_width']) ? $settings['total_width'] : '') . '" class="short-text" />';
        ...
        echo '<li class="field"><label for="off_set" class="coltab">' . __('Number of posts to skip:', 'wp-latest-posts') . '</label>' .
            '<input id="off_set" type="text" name="wplp_off_set" value="' . htmlspecialchars(isset($settings['off_set']) ? $settings['off_set'] : '') . '" class="short-text" />';
        ...
        $all_themes = (array)WPLP_Admin::themeLister();
    ...
    private function displayContentSourceTab() {
        ...
        <ul class="horizontal">
            <?php $idx = 0; ?>
            <?php foreach ($tabs as $tabhref => $tab) : ?>
                <li><input type="radio" name="wplp_source_type" id="sct<?php echo ++$idx; ?>" value="<?php echo $tab['value']; ?>" class="source_type_sel with-gap" <?php echo (isset($source_type_checked[$tab['value']]) ? $source_type_checked[$tab['value']] : ""); ?> />
    ...
    private function displayContentSourceCategoryTab()
    {
        ...
        echo '<li><input id="cat_all" type="checkbox" name="wplp_source_category[]" value="_all" ' . (isset($source_cat_checked['_all']) ? $source_cat_checked['_all'] : '') . ' />' .
        ...
        echo '</ul></li>';
        if (class_exists('WPLP_Addon_Admin') && is_plugin_active('advanced-custom-fields/acf.php')) {
            $display = false;
        ...
        if (!class_exists('WPLP_Addon_Admin')) {
            echo '</ul><div class="card wplp_notice light-blue notice notice-success is-dismissible below-h2" >' .
                '<div class="card-content white-text">' .
                __(
    ...
    private function displayContentSourcePageTab()
    {
        ...
        if (!class_exists('WPLP_Addon_Admin')) {
            echo '<li><input id="pages_all" type="checkbox" name="wplp_source_pages[]" value="_all" checked="checked"  disabled="disabled" />' .
