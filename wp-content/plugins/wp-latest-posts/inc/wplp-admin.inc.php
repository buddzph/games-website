...
class WPLP_Admin
{
    ...
    public function __construct($opts)
    {
        ...
        /** Setup default image * */
        $this->_field_defaults['default_img_previous'] = plugins_url(DEFAULT_IMG, dirname(__FILE__));

        $this->_field_defaults['default_img'] = plugins_url(DEFAULT_IMG, dirname(__FILE__));
        ...
        add_filter('post_updated_messages', array($this, 'wplp_custom_update_messages'));
        if (is_admin()) {
            if (is_multisite()) {
    ...
    public function change_cat_multisite()
    {
        ...
        if (strpos($type, 'post') !== false) {
            ...
            $output .= '<li><input id="cat_all" type="checkbox" name="wplp_source_category[]" value="_all" ' . (isset($source_cat_checked['_all']) ? $source_cat_checked['_all'] : '') . ' />' .
                '<label for="cat_all" class="post_cb">All</li>';
            foreach ($cats as $k => $cat) {
                $output .= '<li><input id="ccb_' . $k . '" type="checkbox" name="wplp_source_category[]" value="' . $k . '_' .
            ...
            foreach ($cats as $k => $page) {
                $output .= '<li><input id="pcb_' . $k . '" type="checkbox" name="wplp_source_pages[]" value="' . $k . '_' .
            ...
            foreach ($cats as $k => $tag) {
                $output .= '<li><input id="tcb_' . $k . '" type="checkbox" name="wplp_source_tags[]" value="' . $k . '_' .
    ...
    public function wplp_custom_update_messages($messages)
    {
    ...
    function addFrontScript()
    {
        ...
        wp_enqueue_script(
            'wplp-front', plugins_url(MAIN_FRONT_SCRIPT, dirname(__FILE__)), array('jquery'), '0.1', true
        );
    }
            ...
            if (preg_match('/^wplp_/', $field_name)) {
                ...
                if (is_array($field_value)) {
                    ...
                    if (isset($this->_enforce_fields[$field_name])) {
                        ...
                        if (FILE_UPLOAD == $this->_enforce_fields[$field_name]) {
                            //Do nothing I guess.
                        }
                        if (LI_TO_ARRAY == $this->_enforce_fields[$field_name]) {
                            if ($field_value) {
    ...
    public function loadAdminScripts($hook)
    {
        ...
        wp_enqueue_script(
            'wplp-easing', plugins_url('js/materialize/jquery.easing.1.3.js', dirname(__FILE__)), array('jquery'), '0.1', true
        );
        wp_enqueue_script(
            'wplp-velocity', plugins_url('js/materialize/velocity.min.js', dirname(__FILE__)), array('jquery'), '0.1', true
        );
        wp_enqueue_script(
            'wplp-tabs', plugins_url('js/materialize/tabs.js', dirname(__FILE__)), array('jquery'), '0.1', true
        );
        wp_enqueue_script(
            'wplp-dropdown', plugins_url('js/materialize/dropdown.js', dirname(__FILE__)), array('jquery'), '0.1', true
        );
        wp_enqueue_script(
            'wplp-form', plugins_url('js/materialize/forms.js', dirname(__FILE__)), array('jquery'), '0.1', true
        );
        wp_enqueue_script(
            'wplp-dropify', plugins_url('js/dropify/js/dropify.min.js', dirname(__FILE__)), array('jquery'), '0.1', true
        );
        wp_enqueue_script(
            'wplp-picker', plugins_url('js/materialize/picker.js', dirname(__FILE__)), array('jquery'), '0.1', true
        );
        wp_enqueue_script(
            'wplp-picker-date', plugins_url('js/materialize/picker.date.js', dirname(__FILE__)), array('jquery'), '0.1', true
        );
        wp_enqueue_script(
            'wplp-back', plugins_url('js/wplp_back.js', dirname(__FILE__)), array('jquery'), '0.1', true
        );
        wp_enqueue_script('wp-color-picker');

        wp_enqueue_script('wplp-newColorPicker', plugins_url('js/wplp_newColorPicker.js', dirname(__FILE__)), array('jquery'), '0.1', true
        );

        /** add codemirror js */
        wp_enqueue_script('wplp-codemirror', plugins_url('codemirror/lib/codemirror.js', dirname(__FILE__)), array('jquery'), '0.1', true
        );
        /** mode css */
        wp_enqueue_script('wplp-codemirrorMode', plugins_url('codemirror/mode/css/css.js', dirname(__FILE__)), array('jquery'), '0.1', true
        );
    ...
    public function setupCustomMetaBoxes()
    {
        ...
        add_meta_box(
            'wplpnavtabsbox', __('WP Latest Posts Block Settings', 'wp-latest-posts'), array(new WPLP_Block_Settings($this->_field_defaults), 'editorTabs'), CUSTOM_POST_NEWS_WIDGET_NAME, 'normal', 'core'
        );
    }
    ...
    {
        add_submenu_page(
            'edit.php?post_type=wplp-news-widget', __('Configuration', 'wp-latest-posts'), __('Configuration', 'wp-latest-posts'), 'activate_plugins', 'translation-wpfpn', array(new WPLP_Views_Config($this->version), 'displayConfigTab')
        );
    }
        ...
        <div id="wplp-popup-wrap" class="media-modal wp-core-ui" style="display:none">
            ...
            <div id="wplp-select-content" class="media-modal-content">
                ...
                <div id="wplp_widgetlist" style="margin:50px auto;">
                    <?php if ($widgets = get_posts(array('post_type' => CUSTOM_POST_NEWS_WIDGET_NAME, 'posts_per_page' => -1))) : ?>
