...
class bt_bb_masonry_post_grid extends BT_BB_Element {
	...
	static function bt_bb_get_grid_callback() {
		bt_bb_masonry_post_grid::dump_grid( intval( $_POST['number'] ), sanitize_text_field( $_POST['category'] ), $_POST['show'] );
		die();
	}
	...
	static function dump_grid( $number, $category, $show ) {
		...
		foreach( $posts as $item ) {
			...
			$output .= '<div class="bt_bb_grid_item" data-hw="' . $hw . '" data-src="' . $img_src . '"><div class="bt_bb_grid_item_inner">';
