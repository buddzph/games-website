...
function bt_bb_import_menu() {
	add_submenu_page( 'tools.php', __( 'Bold Builder Import', 'bt_bb_wordpress_importer' ), __( 'Bold Builder Import', 'bt_bb_wordpress_importer' ), 'manage_options', 'bt_bb_import', 'bt_bb_import' );
}
			...
			<table class="form-table">
				...
				<tr>
					<td class="bt_bb_import_xml_container"><?php
						...
						foreach ( $files as $f ) {
							$n++;
							echo '<div class="bt_bb_import_xml" data-file="' . $f . '">' . $n . '. ' . $f . '</div>';
						}
					?></td>
				</tr>
				<tr>
...
class BT_BB_WP_Import extends WP_Importer {
	...
	function import_start( $file ) {
		if ( ! is_file($file) ) {
			echo '<p><strong>' . __( 'Sorry, there has been an error.', 'bt_bb_wordpress_importer' ) . '</strong><br />';
			echo __( 'The file does not exist, please try again.', 'bt_bb_wordpress_importer' ) . '</p>';
			$this->end = true;
			die();
		}
		...
		if ( strpos( $bt_bb_import_data, '[woocommerce' ) !== false && ! in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
			echo '<p><strong>' . __( 'Error. This demo contains WooCommerce data. Please install and activate WooCommerce.', 'bt_bb_wordpress_importer' ) . '</strong><br />';
			$this->end = true;
			die();
		}		
	...
	function handle_upload() {
		...
		if ( isset( $file['error'] ) ) {
			...
			echo '<p><strong>' . __( 'Sorry, there has been an error.', 'bt_bb_wordpress_importer' ) . '</strong><br />';
			printf( __( 'The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'bt_bb_wordpress_importer' ), esc_html( $file['file'] ) );
			echo '</p>';
			return false;
		}
		...
		if ( $this->version > $this->max_wxr_version ) {
			...
			printf( __( 'This WXR file (version %s) may not be supported by this version of the importer. Please consider updating.', 'bt_bb_wordpress_importer' ), esc_html($import_data['version']) );
			echo '</strong></p></div>';
		}
	...
	function get_authors_from_import( $import_data ) {
		...
		// no author information, grab it from the posts
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					printf( __( 'Failed to import author %s. Their posts will be attributed to the current user.', 'bt_bb_wordpress_importer' ), esc_html( $post['post_author'] ) );
					echo '<br />';
					continue;
				}
...
<?php if ( $this->allow_create_users() ) : ?>
	<p><?php printf( __( 'If a new user is created by WordPress, a new password will be randomly generated and the new user&#8217;s role will be set as %s. Manually changing the new user&#8217;s details will be necessary.', 'bt_bb_wordpress_importer' ), esc_html( get_option('default_role') ) ); ?></p>
<?php endif; ?>
	<ol id="authors">
...
<?php
	...
	function author_select( $n, $author ) {
		...
		wp_dropdown_users( array( 'name' => "user_map[$n]", 'multi' => true, 'show_option_all' => __( '- Select -', 'bt_bb_wordpress_importer' ) ) );
		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					printf( __( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'bt_bb_wordpress_importer' ), esc_html($this->authors[$old_login]['author_display_name']) );
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( __( 'Failed to import category %s', 'bt_bb_wordpress_importer' ), esc_html($cat['category_nicename']) );
				if ( defined('BT_BB_IMPORT_DEBUG') && BT_BB_IMPORT_DEBUG )
	...
	function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( __( 'Failed to import post tag %s', 'bt_bb_wordpress_importer' ), esc_html($tag['tag_name']) );
				if ( defined('BT_BB_IMPORT_DEBUG') && BT_BB_IMPORT_DEBUG )
					echo ': ' . $id->get_error_message();
				echo '<br />';
				continue;
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( __( 'Failed to import %s %s', 'bt_bb_wordpress_importer' ), esc_html($term['term_taxonomy']), esc_html($term['term_name']) );
				if ( defined('BT_BB_IMPORT_DEBUG') && BT_BB_IMPORT_DEBUG )
					echo ': ' . $id->get_error_message();
				echo '<br />';
				continue;
	...
	function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				printf( __( 'Failed to import &#8220;%s&#8221;: Invalid post type %s', 'bt_bb_wordpress_importer' ),
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				...
				if ( is_wp_error( $post_id ) ) {
					printf( __( 'Failed to import %s &#8220;%s&#8221;', 'bt_bb_wordpress_importer' ),
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							printf( __( 'Failed to import %s %s', 'bt_bb_wordpress_importer' ), esc_html($taxonomy), esc_html($term['name']) );
							if ( defined('BT_BB_IMPORT_DEBUG') && BT_BB_IMPORT_DEBUG )
	...
	function process_menu_item( $item ) {
		...
		if ( ! $menu_id ) {
			printf( __( 'Menu item skipped due to invalid menu slug: %s', 'bt_bb_wordpress_importer' ), esc_html( $menu_slug ) );
			echo '<br />';
			return;
		} else {
	...
	function process_attachment( $post, $url ) {
		if ( ! $this->fetch_attachments )
			return new WP_Error( 'attachment_processing_error',
				__( 'Fetching attachments is not enabled', 'bt_bb_wordpress_importer' ) );
	...
	function header() {
		echo '<div class="wrap">';
		screen_icon();
		echo '<h2>' . __( 'Import WordPress', 'bt_bb_wordpress_importer' ) . '</h2>';
	...
	function greet() {
		...
		echo '<p>'.__( 'Howdy! Upload your WordPress eXtended RSS (WXR) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, and tags into this site.', 'bt_bb_wordpress_importer' ).'</p>';
		echo '<p>'.__( 'Choose a WXR (.xml) file to upload, then click Upload file and import.', 'bt_bb_wordpress_importer' ).'</p>';
